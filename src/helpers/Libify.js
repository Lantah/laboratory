...
Libify.Operation = function (type, opts) {
  ...
  if (
    typeof opFunction === "undefined" ||
    has(Libify.Operation, "opFunction")
  ) {
...
Libify.Operation.createAccount = function (opts) {
  assertNotEmpty(
    opts.destination,
    "Create Account operation requires destination",
  );
  assertNotEmpty(
    opts.startingBalance,
    "Create Account operation requires starting balance",
  );
...
Libify.Operation.pathPaymentStrictSend = function (opts) {
  assertNotEmpty(
    opts.sendAsset,
    "Path Payment Strict Send operation requires sending asset",
  );
  assertNotEmpty(
    opts.sendAmount,
    "Path Payment Strict Send operation requires send amount",
  );
  assertNotEmpty(
    opts.destination,
    "Path Payment Strict Send operation requires destination",
  );
  assertNotEmpty(
    opts.destAsset,
    "Path Payment Strict Send operation requires destination asset",
  );
  assertNotEmpty(
    opts.destMin,
    "Path Payment Strict Send operation requires the minimum destination amount",
  );
...
Libify.Operation.pathPaymentStrictReceive = function (opts) {
  assertNotEmpty(
    opts.sendAsset,
    "Path Payment Strict Receive operation requires sending asset",
  );
  assertNotEmpty(
    opts.sendMax,
    "Path Payment Strict Receive operation requires max send",
  );
  assertNotEmpty(
    opts.destination,
    "Path Payment Strict Receive operation requires destination",
  );
  assertNotEmpty(
    opts.destAsset,
    "Path Payment Strict Receive operation requires destination asset",
  );
  assertNotEmpty(
    opts.destAmount,
    "Path Payment Strict Receive operation requires the destination amount",
  );
...
Libify.Operation.allowTrust = function (opts) {
  ...
  assertNotEmpty(
    opts.authorize,
    "Allow Trust operation requires authorization setting",
  );
...
Libify.Operation.accountMerge = function (opts) {
  assertNotEmpty(
    opts.destination,
    "Account Merge operation requires destination",
  );
...
Libify.Operation.manageSellOffer = function (opts) {
  assertNotEmpty(
    opts.selling,
    "Manage Sell Offer operation requires selling asset",
  );
  assertNotEmpty(
    opts.buying,
    "Manage Sell Offer operation requires buying asset",
  );
...
Libify.Operation.manageBuyOffer = function (opts) {
  assertNotEmpty(
    opts.selling,
    "Manage Buy Offer operation requires selling asset",
  );
  assertNotEmpty(
    opts.buying,
    "Manage Buy Offer operation requires buying asset",
  );
  assertNotEmpty(
    opts.buyAmount,
    "Manage Buy Offer operation requires buyAmount",
  );
...
Libify.Operation.createPassiveSellOffer = function (opts) {
  assertNotEmpty(
    opts.selling,
    "Create Passive Sell Offer operation requires selling asset",
  );
  assertNotEmpty(
    opts.buying,
    "Create Passive Sell Offer operation requires buying asset",
  );
  assertNotEmpty(
    opts.amount,
    "Create Passive Sell Offer operation requires amount",
  );
  assertNotEmpty(
    opts.price,
    "Create Passive Sell Offer operation requires price",
  );
...
Libify.Operation.claimClaimableBalance = function (opts) {
  assertNotEmpty(
    opts.balanceId,
    "Claim Claimable Balance operation requires claimable balance ID",
  );
...
Libify.Operation.createClaimableBalance = function (opts) {
  assertNotEmpty(
    opts.asset,
    "Create Claimable Balance operation requires asset",
  );
  assertNotEmpty(
    opts.amount,
    "Create Claimable Balance operation requires amount",
  );
  assertNotEmpty(
    opts.claimants,
    "Create Claimable Balance operation requires at least one claimant",
  );

  let libifiedClaimants = map(opts.claimants, (claimant) => {
    if (!claimant || !claimant.destination) {
      throw new Error(
        "Create Claimable Balance operation requires claimant destination",
      );
    } else if (!claimant.predicate) {
      throw new Error(
        "Create Claimable Balance operation requires claimant predicate",
      );
    }
...
Libify.Operation.revokeSponsorship = function (opts) {
  ...
  switch (opts.revoke.type) {
    ...
    case "claimableBalance":
      assertNotEmpty(
        opts.revoke.fields.balanceId,
        "Claimable balance ID should be set",
      );
...
Libify.Operation.clawback = function (opts) {
  ...
  assertNotEmpty(
    opts.from,
    "Clawback operation requires account from which the asset is clawed back",
  );
...
Libify.Operation.clawbackClaimableBalance = function (opts) {
  assertNotEmpty(
    opts.balanceId,
    "Clawback Claimable Balance operation requires claimable balance ID",
  );
...
Libify.Operation.setTrustLineFlags = function (opts) {
  ...
  assertNotEmpty(
    opts.trustor,
    "Set Trust Line Flags operation requires trustor",
  );
    ...
    each(operations, (op, index) => {
      try {
        transaction = transaction.addOperation(
          Libify.Operation(op.name, op.attributes),
        );
      } catch (e) {
...
Libify.signTransaction = function (
  txXdr,
  signers,
  networkPassphrase,
  ledgerWalletSigs,
