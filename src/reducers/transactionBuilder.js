...
import { BASE_FEE } from "lantah-sdk";
import {
  UPDATE_ATTRIBUTES,
  ...
  switch (action.type) {
    case LOAD_STATE:
      if (action.slug === SLUG.TXBUILDER) {
        if (action.queryObj.params) {
          ...
          return (
            rehydrate(action.queryObj.params).operations || defaultOperations
          );
        }
    ...
    case "UPDATE_OPERATION_ATTRIBUTES":
      return updateOperation(state, action.opId, {
        attributes: assign(
          {},
          getAttributes(state, action.opId),
    ...
    case LOAD_STATE:
      if (action.queryObj.params) {
        return assign(
          {},
          defaultAttributes,
          rehydrate(action.queryObj.params).attributes,
        );
      }
      break;
  ...
  switch (action.type) {
    case LOAD_STATE:
      if (action.queryObj.params) {
        return assign(
          {},
          defaultFeeBumpAttributes,
          rehydrate(action.queryObj.params).feeBumpAttributes,
        );
      }
      break;
  ...
  switch (action.type) {
    case LOAD_STATE:
      if (action.queryObj.params) {
        ...
        return [TX_TYPES.REGULAR, TX_TYPES.FEE_BUMP].indexOf(prevTxType) > -1
          ? prevTxType
          : TX_TYPES.REGULAR;
      }
      break;
